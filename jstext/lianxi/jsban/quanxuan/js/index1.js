define(function(require, exports, module) {
	var P = {
		...
		piu:function(){
			var oneZ = document.getElementById('onez');
			var oneDiv = document.getElementById('onediv')
			var oneX = oneDiv.getElementsByTagName('input')
			oneZ.onclick = function(){
				if(P.attr(oneX[0],'checked')==false){
					alert()
					console.log(P.attr(oneX[0],'checked'))
					P.attr(oneX[0],'checked',true);
				}else{
					alert();
					P.attr(oneX[0],'checked',false)
				}
			}
		}
		//老实说用jq就可以啦
	...
	module.exports = {
		...
		//           obj.addEventListener(type,fn,true);dow
		//      }else {
